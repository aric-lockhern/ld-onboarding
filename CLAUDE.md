...
- **`getLastRow()` counts formatting, not records.** A Clients tab with 500 styled empty rows reports 501, so every list built off it filters on the ID cell. `unassignedClients_` did not, and the Team page announced "500 clients with no owner" with 500 blank names under it. Any new read of a tab needs the same filter.
